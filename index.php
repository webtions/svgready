...
// Process only if user submitted data.
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $converter = new SvgConverter($_POST);
    $converter->process();

    // Get results and error message.
    $results        = $converter->get_results();
    $error_message  = $converter->get_error() ?? '';
}

// Extract variables for template compatibility.
