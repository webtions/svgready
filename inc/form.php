...
<section class="input-section" aria-labelledby="input-heading">
    ...
    <form method="post" aria-describedby="form-instructions">
        ...
        ><?php echo isset($_POST['svg']) ? svgready_escape_html($_POST['svg']) : ''; ?></textarea>

        <fieldset class="opts">
            <legend class="sr-only">Conversion options</legend>

            <label>
                <input type="checkbox" name="strip_wh" <?php echo ! empty($strip_root_wh) ? 'checked' : ''; ?>>
                Strip width/height on root &lt;svg&gt;
            </label>

            <label>
                <input type="checkbox" name="strip_class" <?php echo ! empty($strip_root_class) ? 'checked' : ''; ?>>
                Remove class on root &lt;svg&gt;
            </label>

            <label>
                <input type="checkbox" name="show_base64" <?php echo ! empty($show_base64) ? 'checked' : ''; ?>>
                Also show base64
            </label>
        </fieldset>
        ...
        <?php
        if (! empty($input_svg) && ! empty($normalized) ) :
            ...
            <p class="optimize-result">
                ...
                <span class="change <?php echo $percent > 0 ? 'reduced' : ''; ?>">
                    (<?php echo $percent >= 0 ? '-' . $percent : '+' . abs($percent); ?>%)
                </span>
            </p>
        <?php endif; ?>
    </form>
